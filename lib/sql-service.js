...
async function getObservations(query) {
  ...
  switch (query.path) {
    case undefined:
      ...
      sql += _.isEmpty(query.params) ? ' ORDER BY PM25_AQI DESC' : ' WHERE ?'
      break;
    case 'aac':
    ...
    default:
      ...
      sql += _.isEmpty(query.params) ? ' ORDER BY PM25_AQI DESC' : ' AND ?'
  }
  sql = mysql.format(sql, query.params).replace(/(', `)/g, '\' AND `')
  sql += _.isEmpty(query.params) ? '' : ' ORDER BY PM25_AQI DESC'
...
async function insertIntoAqObs(data) {
  let p = await pool
  let res = await p.query('INSERT IGNORE INTO aq_obs VALUES ?', [data])
  console.log(res)
  return res
}
